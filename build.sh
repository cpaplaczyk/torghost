...
gcc -Os -I /usr/include/$(python3 -V | awk '{print "Python"$2}' | cut -d '.' -f 1-2 | tr -d ' ') -o torghost torghost.c -l$(python3 -V | awk '{print "Python"$2}' | cut -d '.' -f 1-2 | tr -d ' ') -lpthread -lm -lutil -ldl
if [ $? -eq 0 ]; then
